...
if ( "undefined" != typeof(PhpDebugBar) ) {
	...
	var ProfilerWidget = PhpDebugBar.Widget.extend({
		tagName: 'div',
		className: 'profilerwidget',
		render: function() {
			this.bindAttr('data', function() {
				this.$el.html(this.get('data'));
			});

			this.$badge = $('<span />').addClass('badge').appendTo(this.$tab);
	...
	//phpdebugbar.createTab("template", new ProfilerWidget());

	//phpdebugbar.addDataMap({"template": ["profiler.html", []]});
}
